...
testInTournamentAsAdmin(
  ...
  async ({ page }) => {
    ...
    await page.getByRole("spinbutton", { name: "Group phase rounds" }).fill("2");
    await page.getByRole("spinbutton", { name: "Groups count" }).fill("2");
    await page.locator("#total_teams").fill("8");
    await page.getByRole("spinbutton", { name: "Total teams Advancing teams" }).fill("4");
    await page.getByRole("button", { name: "Plan tournament" }).click();

    await expect(page.getByRole("heading", { name: "Tournament Ladder" })).toBeVisible();

    await page.getByRole("link", { name: "Overview" }).click();

    // THEN
    await expect(page.getByText("Team leaderboard (to be implemented)")).toBeVisible();
    await expect(page.getByText("Upcoming debates (to be implemented)")).toBeVisible();
    await expect(page.getByText("Event log (to be implemented)")).toBeVisible();
    await expect(page.getByText("Tournament rules (to be implemented)")).toBeVisible();
  },
);